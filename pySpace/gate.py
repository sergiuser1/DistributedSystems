...
class Gate(threading.Thread):
    ...
    def _handle_GETP(self, request):
        result = self.space.spaces[request['target']]['space'].getp(tuple(request['tuple']))
        code = 0
        message = 'OK'
        response = self._message(request)
        response['code'] = code
    ...
    def _handle_QUERYP(self, request):
        result = self.space.spaces[request['target']]['space'].queryp(tuple(request['tuple']))
        code = 0
        message = 'OK'
        response = self._message(request)
        response['code'] = code
